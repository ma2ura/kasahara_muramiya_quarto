...
ME_sorted_portfolio <- annual_data |>
  ...
  drop_na() |> # 欠損行を削除
  summarize(
    .by = c(month_ID, ME_rank10), # 月次と時価総額ランクでグループ化
...
ME_cross_sectional_return <- ME_sorted_portfolio |>
  summarize(
    .by = ME_rank10, # 時価総額ランクでグループ化
    mean_Re = mean(Re) # 月次超過リターンの平均値を計算
  ) 
# 作図
...
BEME_sorted_portfolio <- annual_data |>
  ...
  drop_na() |>
  summarize(
    .by = c(month_ID, BEME_rank10), # 月次とBEMEランクでグループ化
    # 月次超過リターンの平均値を計算
    Re = mean(Re)
  )
...
BEME_sorted_portfolio <- annual_data |>
  ...
  mutate(
    .by = c(month_ID, BEME_rank10), # 月次とBEMEランクでグループ化
    # 各ポートフォリオで保有比率を計算
    w = lagged_ME / sum(lagged_ME) # 保有費率
  ) |>
  summarize(
    .by = c(month_ID, BEME_rank10), # 月次とBEMEランクでグループ化
    # 時価総額加重の月次超過リターンを計算
...
```{r ch06_22, filename="グループごとの線形回帰 (2)"}
CAPM_results_nested <- ME_sorted_portfolio |>
  nest(.by = ME_rank10) |> 
  mutate(
    # ネイティブ・ラムダ式 \(x) を用いて、各データフレームに lm と tidy を適用
    CAPM_summary = purrr::map(data, \(df) broom::tidy(lm(Re ~ R_Me, data = df)))
  ) |>
  select(-data) |>
...
annual_data <- annual_data |>
  ...
  mutate(
    .by = year, # 年度ごとに
    ME_rank2 = as.factor(ntile(lagged_ME, 2)) # lagged_MEを2つのグループに分類
  )
```
...
annual_data |>
  summarize(
    .by = c(ME_rank2, BEME_rank3), # ランクごとにグループ化
    FF_portfolio_type = FF_portfolio_type[1],
...
FF_portfolio <- annual_data |>
  ...
  summarize(
    .by = c(month_ID, FF_portfolio_type), # 月次とポートフォリオの種類でグループ化
    ME_rank2 = ME_rank2[1],
...
FF_portfolio_mean_return <- FF_portfolio |>
  mutate(Re = R - R_F) |>
  summarize(
    .by = FF_portfolio_type, # ポートフォリオの種類でグループ化
    ME_rank2 = ME_rank2[1],
    BEME_rank3 = BEME_rank3[1],
    mean_Re = mean(Re)
...
ggplot(FF_portfolio_mean_return) +
  ...
  # x軸をBEME_rank3, y軸をmean_Reに, ME_rank2のサブグループで色分け
...
ME_sorted_portfolio <- ME_sorted_portfolio |>
  dplyr::select(-c(R_Me, R_M)) |>
  full_join(factor_data, by = "month_ID")
  # inner_join(factor_data, by = "month_ID") # 実証期間を厳密に一致させるためinner_joinを推奨

FF3_results <- ME_sorted_portfolio |>
  dplyr::reframe(
    broom::tidy(
      lm(Re ~ R_Me + SMB + HML, data = dplyr::pick(everything()))
    ),
    .by = ME_rank10
  )
```
