...
**市場ポートフォリオ**(market portfolio)とは，**市場に存在する全ての危険資産を時価総額比率で保有したポートフォリオ**をいいます([2章を参照](chapter02.qmd#chap02_CAPM))。
...
BEME_sorted_portfolio <- annual_data |>
  mutate(
    .by = year, # 年度ごとに
    lagged_BEME = lagged_BE / lagged_ME, # 簿価時価比率を計算
    BEME_rank10 = ntile(lagged_BEME, 10) # 10個のグループに分類
  ) |>
  select(year, firm_ID, BEME_rank10, lagged_ME) |>
  inner_join(monthly_data, by = c("year", "firm_ID")) |>
  drop_na() |>
  mutate(
    .by = c(month_ID, BEME_rank10), # 月次とBEME_rank10でグループ化
    w = lagged_ME / sum(lagged_ME) # 時価総額加重のウェイトを計算
  ) |>
  summarize(
    .by = c(month_ID, BEME_rank10), # 月次とBEME_rank10でグループ化
    Re = weighted.mean(Re, lagged_ME) # 時価総額加重平均で月次超過リターンを計算
  )
...
このCAPMを単回帰式で表現すると次のようになります。

$$
R_{i,t}^e = \beta_i \times R_{M,t}^e + \varepsilon_{i,t}
$$
ここで、$R^e_{i,t} = R_{i,t} - R_{F,t}$で計算されます。
...
回帰分析における最小二乗法の仮定と同じです。
こうすることで、CAPM式を線形回帰モデルで表現できるので、最小二乗法による$\beta_i$の推定が可能となります。

### 時系列回帰

CAPM式は任意の$i$証券で成立するモデルのため、ポートフォリオにも応用できます。
つまりあるポートフォリオの超過リターン$R_{P,t}^e$を、市場ポートフォリオの超過リターン$R_{M,t}^e$と、そのポートフォリオに対するベータ係数$\beta_P$の積で説明することができます。

$$
R_{P,t}^e = \alpha _P + \beta_P R_{M,t}^e + \varepsilon_{P,t}
$$

CAPMの式と比較すると、<span class="markp">切片である$\alpha_P$が追加されている</span>ことが分かります。
<span class="marky">もし証券市場にCAPMの関係が成立しているなら、$\alpha_P$はゼロとなっているはずです。</span>
...
最後に、テキストの方法とは異なりますが、`dplyr1.0.0`から追加された`dplyr::relocate()`関数を用いて、`R_Me`を最終列に移動しています。
...
:::{.column-margin}
`dplyr::reframe()`関数は、グループごとにデータフレームを作成する関数です。
`dplyr::summarise()`関数は、グループごとに要約統計量などを計算する関数で、グループごとに1つの数値を返すことを想定しているのに対し、
`dplyr::reframe()`関数は、グループごとに長さが2以上のベクトルを返すことができる関数です。
:::
...
CAPM_results <- ME_sorted_portfolio |>
  dplyr::reframe(
    ...
    broom::tidy( # lm()関数の結果をtidy()関数でデータフレームに変換
      lm(Re ~ R_Me, data = cur_data())
    )
  )
...
:::{.column-margin}
`cur_data()`関数は、現在のグループのデータフレームを返す関数です。
`dplyr::reframe()`関数の中で、グループごとに回帰分析を行うために、`lm()`関数に渡すデータフレームを指定する必要がありますが、そのために`cur_data()`関数を用いて、現在のグループのデータフレームを渡しています。
::: 
...
alpha_plot_data <- CAPM_results |>
    dplyr::filter(term == "(Intercept)") |> # 定数項に関する推定結果のみを抽出
    mutate(
      ME_rank10 = factor(ME_rank10)
    )
alpha_plot_data |>     
...
CAPMが成立している世界ではゼロになるはずの切片$\alpha_P$の値をグループごとにみると、時価総額の小さいグループほど$\alpha_P$が大きいことが分かり、CAPMが説明できないサイズ・プレミアムの存在が示唆されます。


```{r ch06_24, filename = "CAPMアルファの統計的な有意性を評価"}
CAPM_results |>
  filter(term == "(Intercept)") |> # 定数項を抽出
  transmute( # 必要な変数を選択して名称変更
    ME_rank10,
    CAPM_alpha = estimate,
    p_value = p.value
  ) |>
  mutate(
    significance = dplyr::case_when( # 統計的に有意な結果を*で強調
      p_value <= 0.01 ~ "***",
      p_value <= 0.05 ~ "**",
      p_value <= 0.10 ~ "*",
      TRUE ~ ""
  ...
  arrange(ME_rank10) # ME_rank10の順に並び替え
```

:::{.column-margin}
教科書では、`rename()`と`select()`を用いて、変数名を変更して必要な変数のみを残す、という方法が紹介されていますが、ここでは、`transmute()`関数を用いて、必要な変数を選択して名称変更を同時に行っています。
`transmute()`関数は、`mutate()`関数と同様に新しい変数を作成する関数ですが、`transmute()`関数は、作成した変数のみを残す関数です。
:::

第1〜第6グループの$\alpha_P$は統計的に有意であることが分かりました。

次に、証券市場線を推定してみます。
定数項$\alpha_P$を加えた線形回帰モデルを推定します。
このとき回帰残差の平均値がゼロとなるため、回帰式は次のようになります。

$$
\bar{R}_{P}^e = \hat{\alpha} _P + \hat{\beta}_P \times \bar{R}_{M}^e 
$$

```{r ch06_25, filename = "証券市場線の推定"}
ME_cross_sectional_return <- CAPM_results |>
  filter(term == "R_Me") |> # R_Meの係数ベータを抽出
  transmute( # 必要な変数を選択して名称変更
    ME_rank10,
    CAPM_beta = estimate
    ) |>
  ...
  full_join( # 超過リターンのデータと結合
    ME_cross_sectional_return,
    by = "ME_rank10"
  )

# 平均超過リターンの平均
mean_R_Me <- factor_data |>
  reframe(mean_R_Me = mean(R_Me, na.rm = TRUE)) |>
  pull()
...
証券市場線から離れた場所に点が分布していることが分かり、CAPMが説明できないサイズ・プレミアムの存在が示唆されます。

### Fama-Frenchの3ファクター・モデル

FF3モデルは、CAPMの式に、サイズ・ファクターとバリュー・ファクターを説明変数として加えたモデルです。

$$
R_{i,t}^e = \beta_{M,i} R_{M,t}^e + \beta_{i}^{SMB} SMB_t + \beta_{i}^{HML} HML_t + \varepsilon_{i,t}
$$

#### 銘柄のランク付け

前年度時価総額に基づいて銘柄を2つのグループに分類し、さらに前年度簿価時価比率に基づいて銘柄を3つのグループに分類します。

まずは、前年度の時価総額に基づいて銘柄を2つのグループに分類します。
`ntile()`関数を用いて、前年度の時価総額`lagged_ME`を2つのグループに分類し、`as.factor()`関数で因子型に変換して`ME_rank2`を作成します。

:::{.column-margin}
`ntile()`関数は、データを指定した数のグループに分類する関数です。
`ntile(x, n)`は、ベクトル`x`を`n`個のグループに分類し、各要素が属するグループ番号を返します。
::: 
...
annual_data <- annual_data |>
  mutate(
    .by = year,
    lagged_ME = if_else(is.na(lagged_BEME), NA_real_, lagged_ME),
    ME_rank2  = factor(
      ntile(lagged_ME, 2), # 前年度時価総額で2グループ      
      levels = c(1, 2),
      labels = c("Small", "Big")
      )
  )
...
annual_data |>
  mutate( # 年度ごとに簿価時価比率のパーセンタイル順位を計算
    .by = year,
...
annual_data$FF_portfolio_type |> table()
```

```{r ch06_30, filename = "Size-BE/MEポートフォリオへの分類 (2)"}
annual_data <- annual_data |>
  mutate(     # ファクター型の変数
    FF_portfolio_type = fct_recode(
      FF_portfolio_type,
      SL = "Small.Low",
      BL = "Big.Low",
      SN = "Small.Mid",
