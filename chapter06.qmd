...
最後に、`dplyr::relocate()`関数を用いて、`R_Me`を最終列に移動しています。
...
このようにして、`ME_sorted_portfolio`には、月次超過リターン`Re`と市場ポートフォリオの月次超過リターン`R_Me`の両方が含まれることになります。
...
ME_sorted_portfolio |>
  ...
  labs(x = "マーケットポートフォリオの超過リターン", y = "小規模株式ポートフォリオの超過リターン") +
  mystyle
```
...
CAPM_results <- ME_sorted_portfolio |>
  dplyr::reframe(
    ...
    broom::tidy(
      lm(Re ~ R_Me, data = pick(everything()))
    )
  )
