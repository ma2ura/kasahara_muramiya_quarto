...
tidyverseとは，R神Wickham氏が基本コンセプトを設定し，tidyデータに対して一貫した記法でデータを扱えるパッケージ群です。
インストールと読み込みは以下の通りです。

```{r ch04_01}
#install.packages("tidyverse") # 初回だけ
library(tidyverse) # 毎回読み込み。めんどくさいときは.Rprofileに書き込む
```
...
いままではcsvデータを読み込むために，基本関数の`read.csv()`を使ってきましたが、より高速かつオプション指定が柔軟なtidyverse関数群の1つである`readr`パッケージの`read_csv()`関数を使います。readとcsvの間がピリオド`.`からアンダースコア`_`に変わっているので注意してください。
