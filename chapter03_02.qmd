...
```{r}
Y <- Re(res[3]) # 多項式の3つ目の解の実数部
(IRR <- Y - 1)
```

今回は，多項式の解が3つしかなかったので，目で実数解を探しましたが，もっと高次の多項式となるとこうはいきません。たとえば，$Y^7$まで出てくる多項式の場合，
...
このように解も7つ出てきます。この中から目で実数解を探すのも面倒なので，実数解のみを取り出す方法を使います。ここで，`Im()`関数を使って虚数部分を取り出し，虚数部分の絶対値が十分に小さいものを実数解として扱います。

```{r}
Re(res[abs(Im(res)) < 1e-8]) - 1 # 実数解のみを抽出
```
...
cal_IRR <- function(CF) {
  ...
  x <- Re(sol[abs(Im(sol)) < 1e-8]) - 1 # 虚部が十分小さい解のみを採用
  return(x)
}
CF <- c(-100, 40, 50, 60) # 例
cal_IRR(CF)
...
calc_irr <- function(cash_flow, initial_investment, max_iter = 100, tol = 1e-10) {
  ...
  # return: IRR (内部収益率)
  npv <- function(r) {
    -initial_investment + sum(cash_flow / (1 + r)^(seq_along(cash_flow)))
  }

  d_npv <- function(r) {
    -sum(seq_along(cash_flow) * cash_flow / (1 + r)^(seq_along(cash_flow) + 1))
  }
  ...
  for (i in seq_len(max_iter)) {
    ...
    if (abs(df) < tol) {
      stop("導関数がほぼ0のため，ニュートン法を継続できません。")
    }

    r_new <- r - f / df
    if (r_new <= -0.999999) {
      stop("IRRの探索中に不適切な割引率が発生しました。初期値を変更してください。")
    }

    if (abs(r_new - r) < tol || abs(npv(r_new)) < tol) {
      return(r_new)
    }
...
この表は，地点，時間，天気，気温の4つの変数があり1つの列をつくっています(ルール1)。大阪12時の天気は雨，気温は24℃といったように1つの行が1つの観測を表しています(ルール2)。このデータには種類の異なる観測はない(ルール3)。また，各セルには1つの値が入っています(ルール4)。よって，これが整然データとなります。
...
```{r}
library(readr)
daily_stock_return <- read_csv("data/ch03_daily_stock_return.csv", show_col_types = FALSE)
```
...
出力結果の1行目をみると，`spc_tbl_ [21 × 3]`とあり，21行・3列のデータであることがわかります。2行目から4行目はデータの構造を表しています。2行目の`date`列は日付型のデータであることを表しています。3行目と4行目の`firm`列は数値型のデータであることを表しています。
...
最小値をとるのがインデックス20の値であることが示されている。このインデックスを使って，`daily_stock_return`データフレームの20行目を取り出します。
...
2つの変数`date`と`stock_return`がそれぞれ5つの要素をもっているデータフレームを作りました。構造を確認してみます。
...
`date`変数が`Date`になりました。`summary()`関数を使ってみます。

```{r}
summary(stock_return_data)
```

日付の`date`も最小値や平均などが出力されていることが分かります。日付型`Date`は、日付としての計算ができるようになります。
...
`tidyverse`パッケージ群に含まれるパッケージに，グラフ作成のための`ggplot2`があります。ここでは，作図用データを作成します。

```{r ch03_54}
initial_cost <- seq(80, 100, by = 1)
IRR <- vapply(initial_cost, function(cost) cal_IRR(c(-cost, 100))[1], numeric(1))
figure_data <- data.frame(initial_cost, IRR) # データフレーム作成
...
```{r}
plot(figure_data$initial_cost, figure_data$IRR, type = "l")
```
...
```{r}
library(tidyverse)
g <- ggplot(figure_data) + aes(x = initial_cost, y = IRR) + geom_line()
print(g)
```
...
$$
FV(n) = \left(1 + \frac{R}{n}\right)^{nT} P.
$$
...
R <- 0.1
T <- 1
compounding_times <- 1:500
FV <- cal_FV(CF, R, n = compounding_times, T = T)
plot(compounding_times, FV, type = "l")
abline(h = CF * exp(R * T), col = "red")
```

将来価値$1000000 \times \mathrm{e}^{0.1}$に収束していることがわかる。
