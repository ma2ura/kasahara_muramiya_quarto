...
たいていの場合，1株当り配当$\text{1株当り配当}_t$はゼロで，調整係数は1となるため，次式のように簡略化できます。
...
$$
で定義され，尖度が3のとき，正規分布と同じ尖度を持つことを示し，3より大きいとき、正規分布よりとがった分布をしていることを示します。
...
**系列相関**（または**自己相関**）とは、同じ時系列データにおいて、時点$t$のリターンと時点$t-k$のリターン（$k$期前のリターン）の間に相関がある状態のことです。例えば、前月のリターンが高ければ今月のリターンも高くなる傾向、というような関係が系列相関です。

*効率的市場仮説*（Efficient Market Hypothesis; EMH）に基づく**半強度の効率的市場**であれば、過去に公開された全ての情報はすでに株価に織り込まれているとされています。したがって、過去の超過リターンは将来の超過リターンと無関係であり、超過リターンに系列相関（自己相関）がないと仮定できます。

しかし、現実には以下のような理由で系列相関が観測されることが知られています：

- **モメンタム効果**：過去のリターンが高かった銘柄は、その後もリターンが高い傾向
- **反転現象**（mean reversion）：極端に高いリターンの後には低いリターンが続く傾向
- **アノマリー**：市場が有効でない期間における異常な価格パターン

これらのため、現実には系列相関があると考えられます。しかし、系列相関を考慮するとモデルが著しく複雑になるため、ここでは最初の学習段階として、独立性の仮定を置いてモデルを単純化します。より高度な分析では、ARMA（自己回帰移動平均）モデルなどで系列相関を明示的に扱う必要があります。
...
母集団から無限個の標本(sample)を抽出して、それぞれの標本平均(sample mean)を計算すると、その標本平均の平均は母集団の期待値$\mu$に一致することが知られています。これを大数の法則(law of large number)といいます。
...
safe_write_csv <- function(df, path) {
  tryCatch(
    write_csv(df, path),
    error = function(e) {
      warning(paste0("CSVの保存に失敗しました: ", path, " / ", conditionMessage(e)))
      invisible(NULL)
    }
  )
}
# safe_write_csv()は、
safe_write_csv(monthly_data, "data/ch05_output1.csv")
safe_write_csv(annual_data, "data/ch05_output2.csv")
```
